...
export default function ExamInterface({ auth, exam, questions }) {
    const [currentIndex, setCurrentIndex] = useState(0);
    const [answers, setAnswers] = useState({});
    const [showRationale, setShowRationale] = useState(false);
    const [timeLeft, setTimeLeft] = useState(exam.timer * 60 || 3600);
    ...
    const submitExam = () => {
        setIsFinished(true);
        // In a real app, we would POST to /api/exams/{id}/submit
        console.log("Submitting answers:", answers);
        router.post(`/exams/${exam.id}/submit`, {
            answers: answers
        });
    };
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans">
            <Head title={`${exam.title} - Online Exam`} />
            
            <header className="fixed top-0 left-0 right-0 bg-white border-b border-slate-100 z-50">
                <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href="/" className="text-xl font-black text-primary">NursingExamSource</Link>
                        ...
                        <div className="p-8">
                            ...
                            <div className="space-y-4">
                                {question.options.map((option, idx) => {
                                    ...
                                    const showFeedback = showRationale && isSelected;
