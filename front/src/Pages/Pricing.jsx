            ...
            <main className="pt-40 pb-32">
                <div className="max-w-7xl mx-auto px-4">
                    <header className="text-center mb-24">
                        <h1 className="text-5xl lg:text-7xl font-black text-slate-950 mb-8 italic">Choose Your <span className="text-primary not-italic">Success Plan</span></h1>
                        <p className="text-xl text-slate-600 font-bold max-w-2xl mx-auto">Flexible options to fit your study schedule and budget.</p>
                    </header>

                    <div className="grid lg:grid-cols-3 gap-10">
                        {plans.map((plan, i) => (
                            <div key={i} className={`relative p-12 rounded-[3.5rem] bg-white border-2 ${plan.isPopular ? 'border-primary shadow-2xl scale-105 z-10' : 'border-slate-100 shadow-xl'} transition-all hover:translate-y-[-8px]`}>
                                {plan.isPopular && (
                                ...
                                <div className="text-slate-400 font-black text-xs uppercase tracking-widest mb-10">{plan.period}</div>
                                
                                <div className="flex items-baseline gap-1 mb-12">
                                    <span className="text-6xl font-black text-slate-950">${plan.price}</span>
                                ...
                                <button className={`w-full py-5 rounded-2xl font-black text-xl transition-all flex items-center justify-center gap-3 ${plan.isPopular ? 'bg-primary text-white hover:bg-primary-dark shadow-primary/30' : 'bg-slate-100 text-slate-900 hover:bg-slate-200'}`}>
                                    {plan.buttonText}
